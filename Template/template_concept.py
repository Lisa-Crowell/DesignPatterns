...
class AbstractClass(metaclass=ABCMeta):
    ...
    @staticmethod
    def step_one():
        """Hooks are normally empty in the abstract class. The implementing class can
        optionally override providing a customer implementation."""

    @staticmethod
    @abstractmethod
    def step_two():
        """An abstract method that must be overridden in the implementing class.
        It has been given '@abstractmethod' decorator so that pylint shows the error"""

    @staticmethod
    def step_three():
        """Hooks can also contain default behavior and can be optionally overridden"""
        print("Step three is a hook that prints this line by default.")

    @classmethod
    def template_method(cls):
        """This is the template method that the subclass will call. The subclass
        (implementing class) doesn't need to override this method since it has/would have
        already optionally overridden the following methods with its own implementation."""
        cls.step_one()
        cls.step_two()
        cls.step_three()
