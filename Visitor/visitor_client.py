...
class IVisitor(metaclass=ABCMeta):
    """An interface that customer Visitors should implement"""
...
class IVisitable(metaclass=ABCMeta):
    """An interface that concrete objects should implement that
    allows the visitor to traverse a hierarchical structure of objects"""
...
class Car(AbstractCarPart, IVisitable):
    ...
    def __init__(self, name):
        ...
        self._parts = [
            ...
            Wheel("BackRight", "GHI-789BL-21", 152)
        ]
...
class PrintPartsVisitor(IVisitor):
    ...
    def visit(element):
        if hasattr(element, "sku"):
            print(f"{element.name}\tab: {element.sku}".expandtabs(6))
